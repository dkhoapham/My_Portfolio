...
import { highlightNav, mobileNav, navHoverAnimation, autoCloseNav, preloadResume } from "./script";
import { resumeViewLink } from "../pages/Resume/Resume";
    ...
    <header className="navClass">
      <div className="navInner">
        <nav className="nav" ref={navRef}>
          ...
          <button className={`nav_toggle ${open ? "active" : ""}`} onClick={() => setOpen(!open)}>
            <span className="bar" />
            <span className="bar" />
            <span className="bar" />
          </button>
          ...
          <div className={`nav_right ${open ? "active" : ""}`}>
            ...
            <div className="resume_bg">
              <button className="resume_btn" onClick={() => navigate("/resume")}>
                <LuFileUser size={20} className="resume" />
              </button>
            </div>
          </div>
        </nav>
